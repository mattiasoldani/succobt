...
root_out_dir="/afs/cern.ch/user/m/msoldani/data_out/splitted"
root_merged_dir="/afs/cern.ch/user/m/msoldani/data_out/merged"
if [ ! -d $ascii_dir ];then
...
for file in $(ls $ascii_dir"/run"$nrun"_"*.dat); do
    ...
    if [[ -e $file && -e $file2 ]] ; then
        ...
        if [ ! -e $root_out_dir/$root_file ];then
            ...
            root -l -b -q 'asciinroot_to_root_pp_ASCIINROOT.cc+('\"$file\"','\"$root_out_dir/$root_file\"','\"$file2\"')'
